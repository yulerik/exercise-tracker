    ...
    return (
        <>  
            <div className='flex flex-row w-full justify-between items-center'>
                ...
                {exercises.map(each => 
                    <div className='collapse w-full' key={each._id}>
                        ...
                        <ul className='collapse-content flex flex-row justify-evenly'>
                            {each.sets.map(set => <li key={set._id} className='badge'><h6>reps: {set.reps}</h6><h6 className='badge badge-info my-2' style={{display: set.weight === 0 && 'none'}}>{` weight: ${set.weight}`}</h6></li>)}
                        </ul>
                        <hr></hr>
                    </div>
                )}
